...
(($, global, undefined?) => {
    ...
    var sizeInfoList, vMId, vmName, vMSize, vMNewSize, subscriptionRegisteredToService,
        getSizeInfoListUrl = "/CmpWapExtensionTenant/ListVmSizes",
        vmOpUrl = "/CmpWapExtensionTenant/VmOp";
    ...
    function fetchSizeInfoList(subscriptionId) {
        $.post(getSizeInfoListUrl, { subscriptionIds: subscriptionId })
            .done(function (data) {
    ...
    function populateConfigureUiElements() {
        ...
        $("#size").change(function () { vmSizeChanged(); });
    }
    ...
    function onVmReSize(item) {
        ...
        resizeConfirmation.setActions([Shell.UI.Notifications.Buttons.yes(
            function () {
                performResize(item);
            }
    ...
    function resizeCommandSend(item, progressOperation) {
        $.post(vmOpUrl, { subscriptionId: subscriptionRegisteredToService[0].id, Opcode: "RESIZE", VmId: vMId, sData: "", iData: 0, Vmsize: vMNewSize })
            .done(function (data) {
